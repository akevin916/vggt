# 數據表 — 自然場景（Sintel / PointOdyssey）

> **域：自然場景。** 醫學域（SCARED / C3VD / 私人內視鏡）的數字在
> [results/medical.md](medical.md)，對外 published 方法的對照在 [results/sota.md](sota.md)。
> **同一個數字只准存在一份表裡**——在這三份之間引用要用連結，不要複製貼上。
>
> 建立 2026-07-15。本檔只放**已經跑出來的數字**與其實驗設定，**不放結論推論**
> （結論在 [status.md](../status.md)，run 的身分與下場在 [experiments.md](../experiments.md)）。
> 每張表開頭的「設定」區塊是重跑該表所需的完整資訊。
>
> 📌 檔內出現的「v3」= 現在的正式版方法（gate 版），見 [method.md](../method.md) 的命名說明。
...
MonST3R flow_loss 可行性、flow 殘差的 ATE 頭空、光流對 pose 的觀測度這三張表，
2026-08-31 搬到 **[topics/ego_flow.md](../topics/ego_flow.md) 附錄 A** ——
它們量的是已收掉的 `L_ego_flow` 線，且產生它們的三支 script 都已刪除、不可重跑。
本檔從此只放 gate 相關的消融。
