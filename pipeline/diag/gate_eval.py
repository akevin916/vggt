...
Run from the repo root:
    python -m pipeline.diag.gate_eval --ckpt logs/<exp>/ckpts/best.pt --all_seqs
    python -m pipeline.diag.gate_eval --ckpt <ckpt> --metrics quality --all_seqs --max_frames 16 --gif
    python -m pipeline.diag.gate_eval --ckpt <ckpt> --dataset po --metrics pose
"""
...
from pipeline.data.datasets.pointodyssey import PointOdysseyDataset
from pipeline.data.motion_mask import DIAG_SEQUENCES, sintel_masks_and_fraction
...
from pipeline.eval.gate_common import oracle_logits_from_masks, po_common_conf, pool_to_patch
from pipeline.eval.gate_vis import save_gate_overlay_gif
from pipeline.eval.metrics_pose import eval_pose_metrics, extrinsics_w2c_to_tum
from pipeline.eval.paths import (
    GATE_BIAS_ABLATION,
    GATE_BIAS_ABLATION_PO,
    GATE_QUALITY,
    default_output_dir,
)
from pipeline.eval.vggt_infer import infer_sequence_chunked, load_vggt_for_eval
