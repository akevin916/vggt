...
def exp_root_from_ckpt(ckpt: str, logs_dir: str | None = None) -> str:
    ...
    return os.path.join(logs_dir, "train", stem)


def exp_name_from_ckpt(ckpt: str, logs_dir: str | None = None) -> str:
    """Experiment name used as the ``<exp>`` level under outputs/."""
    return os.path.basename(exp_root_from_ckpt(ckpt, logs_dir))


def default_output_dir(ckpt: str, tool_name: str, logs_dir: str | None = None) -> str:
    """Default output directory for any post-training script: ``outputs/<tool>/<exp>/``."""
    return os.path.abspath(os.path.join(OUTPUTS_DIR, tool_name, exp_name_from_ckpt(ckpt, logs_dir)))
