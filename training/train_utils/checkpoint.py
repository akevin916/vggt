...
def robust_torch_save(checkpoint: Dict[str, Any], checkpoint_path: str) -> None:
    ...
    if g_pathmgr.exists(checkpoint_path):
        if g_pathmgr.exists(backup_checkpoint_path):
            # Orphaned backup from a previous save that was interrupted after writing
            # checkpoint_path but before this cleanup step ran (checkpoint_path existing
            # right now is proof that save already completed) -- stale, safe to discard.
            logging.warning(
                f"Removing orphaned backup from an interrupted previous save: {backup_checkpoint_path}"
            )
